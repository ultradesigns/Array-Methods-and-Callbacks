...
// function getYears(dataSet, key){
//     const played = dataSet.map(function(dataSet){
//         return dataSet.Year;
//     })
//     return played;
// }
// console.log(getYears(fifaData, "Year"));
...
function getWinners(enterArr){

const newArray = enterArr.map(data => {
...
function getWinnersByYear(getWinners, getYears) {
    ...
    for(let i=0; i < getWinners.length; i++){
        yrWinners.push(`In ${getYears[i]}, ${getWinners[i]} won the world cup`);
    }
    return yrWinners;
}
...
function getAverageGoals(arr){
   const homeAverg = arr.map((element) => element["Home Team Goals"])
   const awayAverg = arr.map((element) => element["Away Team Goals"])
   return `${homeAverg.reduce((a,b)=> a+b) / homeAverg.length + awayAverg.reduce((a,b) => a+b) / awayAverg.length}`;
}
